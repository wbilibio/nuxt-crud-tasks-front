          ...
          <table v-if="list.length" class="min-w-full text-center table">
            <thead class="border-b">
              <tr>
                ...
                <th scope="col" class="th">Data de conclusão</th>
            ...
            <tbody>
              ...
              >
                ...
                <td class="td">{{ task.date_conclusion }}</td>
...
export default {
  ...
  methods: {
    ...mapActions("task", ["listTasks", "deleteTasks"]),
    async remove(id) {
      const res = await this.deleteTasks(id);

      if (res.status === "error") {
        this.showError = true;
        this.msgError = "Não foi possível deletar a tarefa";
